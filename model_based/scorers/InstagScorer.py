...
class InstagScorer(BaseScorer):
    ...
    def evaluate_to_file(self, dataset: str, output_file: str, resume: bool = True) -> str:
        ...
        if resume and os.path.exists(output_file):
            ...
            if done_ids:
                print(f"[InstagScorer] Resume enabled. Found {len(done_ids)} unique completed ids in {output_file}.")
        ...
        with open(dataset, "r", encoding="utf-8", errors="ignore") as f:
            pbar = tqdm(total=num_lines, desc=self.config.get("name", "InstagScorer"))
