...
class HddScorer(BaseScorer):
    ...
    def evaluate_to_file(self, dataset: str, output_file: str, resume: bool = True) -> str:
        ...
        if resume and os.path.exists(output_file):
            ...
            if done_ids:
                print(f"[HddScorer] Resume enabled. Found {len(done_ids)} unique completed ids in {output_file}.")
        ...
        print(f"Using {max_workers} worker(s) for parallel processing")
        pbar = tqdm(total=num_lines, desc=self.config.get("name", "HddScorer"))
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            with open(dataset, "r", encoding="utf-8", errors="ignore") as f:
                for line in f:
                    ...
                    chunk_args.append((line, sample_size))
